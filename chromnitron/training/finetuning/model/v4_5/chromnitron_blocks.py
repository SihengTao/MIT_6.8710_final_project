...
def main():
    length = 10000
    seq = torch.randn(1, length, 5, device='cuda')
    feat = torch.randn(1, length, 1, device='cuda')
    seq = seq.transpose(1, 2)
    feat = feat.transpose(1, 2)
    inputs = (seq, feat)
    prot_input = torch.randn(1, 4096, 2560, device='cuda')
    prot_input = prot_input.transpose(1, 2)
    prot_encoder = ProteinEncoder(2560, hidden = 512, filter_size = 5, num_blocks = 4).cuda()
    prot_encoder(prot_input)
    breakpoint()

    encoder = MultiModalEncoder(1, 512, 9, num_res_blocks=1).cuda()
    decoder = Decoder(512, 256, 3, num_blocks=2).cuda()
    ...
    breakpoint()
    '''
    block = ConvBlock(23, 2, hidden_in = 5, hidden = 128).cuda()
    block_2 = ConvBlock(3, 2, hidden_in = 128, hidden = 128).cuda()
    attn = AttnModule().cuda()
    block_t = ConvTransposeBlock(3, 2, hidden_in = 128, hidden = 128).cuda()
    block_t2 = ConvTransposeBlock(3, 2, hidden_in = 128, hidden = 128).cuda()
    outputs = block(inputs)
    outputs = block_2(outputs)
    outputs = outputs.transpose(1, 2)
    outputs = attn(outputs)
    outputs = outputs.transpose(1, 2)
    print(outputs.shape)
    outputs = block_t(outputs)
    outputs = block_t2(outputs)
    print(outputs.shape)
    breakpoint()
    '''
...
class Decoder(nn.Module):
    def __init__(self, in_channel, hidden = 256, filter_size = 3, num_blocks = 2, output_channel = 1, no_confidence_prediction = False):
        ...
        if self.confidence_prediction:
            ...
            '''
            self.conv_confidence = nn.Sequential(
                ...
                nn.GroupNorm(1, hidden),
                nn.GELU(),
                nn.Conv1d(hidden, output_channel, 1),
            )
            '''
    def forward(self, x):
        x = self.scale(x)
